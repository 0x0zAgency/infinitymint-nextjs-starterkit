{
  ...
  outputs = inputs@{ self, nixpkgs, flake-utils, ... }: flake-utils.lib.eachDefaultSystem (system:
    ...
    in {
      ...
      packages.nextjs_eth = builtProject;
      packages.reactFrontend = reactFrontend;
      packages.default = self.packages.${system}.nextjs_eth;
    }
  );
}
